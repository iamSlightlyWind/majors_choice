...
public class CPUController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (service.equals("insertCPU")) {
            ...
            // CPU c = new CPU(id, baseClock, boostClock, generation, name, generation,
            // socket, cores, threads, baseClock, boostClock, tdp);
            CPU c = new CPU();
            try {
                String fileName = "";
                ...
                if (contentType.indexOf(contentType) >= 0) {
                    DiskFileItemFactory factory = DiskFileItemFactory.builder()
                            .setBufferSize(maxMemSize).setPath(folder)
                            .get();
        ...
        if (service.equals(
            ...
            if (submit == null) {
                ...
                int boostClock = Integer.parseInt(request.getParameter("boostClock"));
                int tdp = Integer.parseInt(request.getParameter("tdp"));
                CPU c = new CPU(id, baseClock, boostClock, generation, name, generation, socket, cores, threads,
